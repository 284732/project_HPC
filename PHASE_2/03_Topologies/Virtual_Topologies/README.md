...
* **Possibilità di ottimizzazione del mapping fisico.** Il parametro `reorder` di `MPI_Cart_create` (sezione 2) consente all'implementazione MPI di **riassegnare** i rank all'interno del nuovo communicator cartesiano, potenzialmente in modo da far coincidere la topologia logica richiesta con la topologia fisica reale dell'interconnessione di rete (ad esempio una rete a toro fisica, comune in molti sistemi HPC), riducendo il numero di hop di rete necessari per le comunicazioni tra vicini logici. Questa ottimizzazione è **facoltativa e dipendente dall'implementazione**: lo standard MPI non obbliga alcuna specifica strategia di remapping, e su molti sistemi (in particolare in ambienti a singolo nodo, come le classiche esecuzioni di sviluppo/test) il remapping non produce alcun effetto misurabile. Va quindi inteso come un'opportunità di ottimizzazione che l'implementazione *può* sfruttare, non come una garanzia prestazionale del programmatore.
...
* `MPI_Cart_create` è essa stessa una **operazione collettiva** (implicitamente, per la natura di come i communicator vengono costruiti in MPI): deve essere invocata da tutti i processi di `comm_old`, con argomenti `ndims`, `dims[]` e `periods[]` consistenti su tutti i chiamanti, per lo stesso motivo di consistenza discusso per le collettive generiche nella guida 03 (sezione 1).
...
La funzione tenta di produrre dimensioni il più possibile **equilibrate tra loro** (fattori il più possibile vicini in valore assoluto), un obiettivo motivato dal fatto che, a parità di numero totale di processi, una griglia "quadrata" (o quasi) minimizza tipicamente il rapporto superficie/volume delle sottodomini in una decomposizione spaziale 2D, riducendo il volume di comunicazione di halo exchange per unità di lavoro computazionale rispetto a una griglia fortemente "allungata" (ad esempio 1×P, che degenera di fatto in una decomposizione 1D a strisce come quella della guida 04b).
...
|---|---|---|
| `MPI_Cart_create` termina il programma con errore | Il prodotto delle `dims[]` fornite eccede il numero di processi disponibili nel communicator di partenza | Verificare `dims[0] * dims[1] * ... == size` (o `≤ size`, gestendo esplicitamente i processi in eccesso) prima della chiamata, tipicamente derivando `dims[]` da `MPI_Dims_create` (sezione 7) invece di valori scelti arbitrariamente |
| Alcuni processi restano con `comm_cart = MPI_COMM_NULL` e crashano alle successive chiamate su quel communicator | Il prodotto delle `dims[]` è strettamente minore del numero di processi in `comm_old`: quei processi in eccesso non entrano nella griglia | Se il numero di processi lanciati non è garantito corrispondere esattamente al prodotto delle dimensioni desiderate, controllare esplicitamente `comm_cart != MPI_COMM_NULL` prima di usarlo, escludendo dal ramo applicativo principale i processi eventualmente esclusi dalla griglia |
...
| `MPI_Cart_sub` produce un raggruppamento opposto a quello atteso (righe invece di colonne o viceversa) | Interpretazione invertita di `remain_dims[i]`: si è impostato a 1 l'indice della dimensione che si intendeva usare per il raggruppamento, invece della dimensione che deve restare distinta all'interno del sotto-communicator (sezione 6) | Ricordare la regola: `remain_dims[i]=1` → quella dimensione resta "viva" (distingue i processi) nel sotto-communicator risultante; `remain_dims[i]=0` → quella dimensione è quella su cui si raggruppa (i processi con la stessa coordinata su quell'asse finiscono insieme) |
| Comunicazione bloccata indefinitamente (deadlock) nell'halo exchange 2D dell'esercizio 2 | Gestione non simmetrica dei quattro vicini, ad esempio usando `MPI_Send`/`MPI_Recv` separati invece di `MPI_Sendrecv` per ciascuna coppia di direzioni opposte (lo stesso scenario generale già discusso in guida 01a sezione 6 e in guida 04b sezione 2.5, qui replicato su quattro direzioni anziché due) | Usare sempre `MPI_Sendrecv` per ciascuna coppia direzione/verso opposto (una chiamata per l'asse verticale, una per l'asse orizzontale), passando direttamente gli output di `MPI_Cart_shift` come parametri `dest`/`source`, così come mostrato in sezione 5 |
